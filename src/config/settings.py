...
class Settings:
    ...
    def __init__(self):
        ...
        self.DEVELOPER_IDS: list[int] = self._parse_int_list(
        ...
        self.ENABLE_STARTUP_VERIFICATION: bool = self._parse_bool(
            os.getenv("ENABLE_STARTUP_VERIFICATION", "true")
        )
        self.ENABLE_AUTO_REPAIR: bool = self._parse_bool(
            os.getenv("ENABLE_AUTO_REPAIR", "true")
        )

        # Feature Flags
        self.ENABLE_MUSIC: bool = self._parse_bool(os.getenv("ENABLE_MUSIC", "false"))
        self.ENABLE_MODERATION: bool = self._parse_bool(
            os.getenv("ENABLE_MODERATION", "true")
        )
        self.ENABLE_FUN: bool = self._parse_bool(os.getenv("ENABLE_FUN", "true"))
        self.ENABLE_UTILITY: bool = self._parse_bool(
            os.getenv("ENABLE_UTILITY", "true")
        )
        self.ENABLE_SAFE_MODE: bool = self._parse_bool(
            os.getenv("ENABLE_SAFE_MODE", "true")
        )

        # Owner Notifications
        self.OWNER_ALERTS_ENABLED: bool = self._parse_bool(
            os.getenv("OWNER_ALERTS_ENABLED", "true")
        )
        self.OWNER_DAILY_DIGEST_ENABLED: bool = self._parse_bool(
            os.getenv("OWNER_DAILY_DIGEST_ENABLED", "true")
        )
        self.OWNER_DAILY_DIGEST_TIME: str = os.getenv(
            "OWNER_DAILY_DIGEST_TIME", "00:00"
        )
        self.OWNER_STATUS_CHANNEL_ID: Optional[int] = self._parse_int(
            os.getenv("OWNER_STATUS_CHANNEL_ID")
        )

        # Health & Monitoring
        self.ENABLE_HEALTH_MONITOR: bool = self._parse_bool(
            os.getenv("ENABLE_HEALTH_MONITOR", "true")
        )
        self.ENABLE_WATCHDOG: bool = self._parse_bool(
            os.getenv("ENABLE_WATCHDOG", "true")
        )
