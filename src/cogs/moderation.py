...
class Moderation(commands.Cog):
    ...
    @app_commands.command(
        name="delete", description="Message deletion commands (Server Owner only)"
    )
    ...
    async def delete(self, interaction: discord.Interaction, action: str):
        ...
        try:
            ...
            await self._error_response(
                interaction, "Failed to process deletion command"
            )
    ...
    async def _delete_all(self, interaction: discord.Interaction):
        ...
        try:
            ...
            new_channel = await channel.clone(
                reason=f"Channel purge by {interaction.user.name}"
            )
    ...
    @app_commands.command(
        name="kick", description="Kick a user from the server (Server Owner only)"
    )
    @app_commands.default_permissions(administrator=True)
    ...
    async def kick(
        ...
        try:
            ...
            if not (is_bot_owner or has_admin_perm or has_staff_role):
                ...
                await interaction.response.send_message(embed=embed, ephemeral=True)
                self.logger.warning(
                    f"Unauthorized kick command attempt by {interaction.user.name} ({interaction.user.id})"
                )
                return
            ...
            if user.id == interaction.user.id:
                embed = embed_helper.error_embed(
                    title="Invalid Action", description="You cannot kick yourself!"
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            if (
                ...
                and interaction.user.id not in settings.OWNER_IDS
            ):
                embed = embed_helper.error_embed(
                    title="Permission Denied",
    ...
    @app_commands.command(
        name="ban", description="Ban a user from the server (Server Owner only)"
    )
    @app_commands.default_permissions(administrator=True)
    ...
    async def ban(
        ...
        try:
            ...
            if not (is_bot_owner or has_admin_perm or has_staff_role):
                ...
                await interaction.response.send_message(embed=embed, ephemeral=True)
                self.logger.warning(
                    f"Unauthorized ban command attempt by {interaction.user.name} ({interaction.user.id})"
                )
                return
            ...
            if user.id == interaction.user.id:
                embed = embed_helper.error_embed(
                    title="Invalid Action", description="You cannot ban yourself!"
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
    ...
    @app_commands.command(
        name="mute", description="Mute a user in the server (Server Owner only)"
    )
    @app_commands.default_permissions(administrator=True)
    ...
    async def mute(
        ...
        try:
            ...
            if not (is_bot_owner or has_admin_perm or has_staff_role):
                ...
                await interaction.response.send_message(embed=embed, ephemeral=True)
                self.logger.warning(
                    f"Unauthorized mute command attempt by {interaction.user.name} ({interaction.user.id})"
                )
                return
            ...
            if user.id == interaction.user.id:
                embed = embed_helper.error_embed(
                    title="Invalid Action", description="You cannot mute yourself!"
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            if not muted_role:
                ...
                for channel in interaction.guild.channels:
                    if isinstance(channel, discord.TextChannel):
                        ...
                        await channel.set_permissions(
                            muted_role, connect=False, speak=False
                        )

            # Mute the user
            await user.add_roles(
                muted_role, reason=f"{reason} | Muted by {interaction.user.name}"
            )
    ...
    async def unmute(
        ...
        try:
            ...
            if not (is_bot_owner or has_admin_perm or has_staff_role):
                ...
                await interaction.response.send_message(embed=embed, ephemeral=True)
                self.logger.warning(
                    f"Unauthorized unmute command attempt by {interaction.user.name} ({interaction.user.id})"
                )
                return
            ...
            await user.remove_roles(
                muted_role, reason=f"{reason} | Unmuted by {interaction.user.name}"
            )
    ...
    async def _error_response(self, interaction: discord.Interaction, message: str):
        """Send error response."""
        embed = embed_helper.error_embed(title="Moderation Error", description=message)
