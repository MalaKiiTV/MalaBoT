...
class MalaBotLogger:
    ...
    def setup_logging(self):
        ...
        latest_handler = logging.FileHandler(
            latest_log_path, mode="w", encoding="utf-8"  # Overwrite each session
        )
...
"""
        ...
        crash_file_path = settings.LOG_FILE.replace(
            "bot.log", f'crash_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        )
        try:
...
"""
    ...
    def log_health_check(
        self, check_type: str, status: str, value: float = None, details: str = None
    ):
...
def log_health_check(
    check_type: str, status: str, value: float = None, details: str = None
):
    """Log health check."""
