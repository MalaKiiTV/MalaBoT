...
class Fun(commands.Cog):
    ...
    async def joke(self, interaction: discord.Interaction):
        ...
        try:
            # Check cooldown
            if not cooldown_helper.check_cooldown(
                interaction.user.id, "joke", COMMAND_COOLDOWNS.get("joke", 5)
            ):
                remaining = cooldown_helper.get_remaining_cooldown(
                    interaction.user.id, "joke"
                )
    ...
    @app_commands.command(
        name="fact", description="Learn something new with a random fact!"
    )
    async def fact(self, interaction: discord.Interaction):
        ...
        try:
            # Check cooldown
            if not cooldown_helper.check_cooldown(
                interaction.user.id, "fact", COMMAND_COOLDOWNS.get("fact", 5)
            ):
                remaining = cooldown_helper.get_remaining_cooldown(
                    interaction.user.id, "fact"
                )
                embed = embed_helper.warning_embed(
    ...
    @app_commands.command(
        name="roast", description="Challenge MalaBoT to a roast battle!"
    )
    ...
    async def roast(
        ...
        try:
            # Check cooldown
            if not cooldown_helper.check_cooldown(
                interaction.user.id, "roast", COMMAND_COOLDOWNS.get("roast", 10)
            ):
                remaining = cooldown_helper.get_remaining_cooldown(
                    interaction.user.id, "roast"
                )
                embed = embed_helper.warning_embed(
    ...
    async def eightball(self, interaction: discord.Interaction, question: str):
        ...
        try:
            ...
            await self._error_response(
                interaction, "The magic 8-ball is feeling shy right now"
            )
    ...
    async def roll(
        self, interaction: discord.Interaction, sides: int = 6, count: int = 1
    ):
