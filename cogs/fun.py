    ...
    async def joke(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not cooldown_helper.check_cooldown(interaction.user.id, 'joke', COMMAND_COOLDOWNS.get('joke', 5)):
                remaining = cooldown_helper.get_remaining_cooldown(interaction.user.id, 'joke')
    ...
    async def fact(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not cooldown_helper.check_cooldown(interaction.user.id, 'fact', COMMAND_COOLDOWNS.get('fact', 5)):
                remaining = cooldown_helper.get_remaining_cooldown(interaction.user.id, 'fact')
                embed = embed_helper.warning_embed(
    ...
    async def roast(self, interaction: discord.Interaction, target: Optional[discord.Member] = None):
        ...
        try:
            ...
            if not cooldown_helper.check_cooldown(interaction.user.id, 'roast', COMMAND_COOLDOWNS.get('roast', 10)):
                remaining = cooldown_helper.get_remaining_cooldown(interaction.user.id, 'roast')
                embed = embed_helper.warning_embed(
